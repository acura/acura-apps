...
public class SaveAccountAction extends AccountAction {
	...
	public Integer updateSubscriptionEndDate(Account account, Date newDate) throws DAOException {
		return getAccountService().updateSubscriptionEndDate(account, newDate);
	}
	private Integer savePortalAccount() throws Exception {
		Account account = populatePortalAccount();
		return getAccountService().saveAccount(account);
	}

	public Account populatePortalAccount() {
		...
		if (getServletRequest().getSession() != null
			...
			accountObject
					.setCreatedBy(getServletRequest().getSession().getAttribute(PortalConstant.ACCOUNT_ID).toString());
			accountObject.setLastUpdatedBy(
					getServletRequest().getSession().getAttribute(PortalConstant.ACCOUNT_ID).toString());
		}
	...
	private String getUniqueAccountId(Integer latestSrNoForAccountId, String ownerName, String accountType) {
		String userId = (ownerName.substring(0, 1).toUpperCase() + accountType.substring(0, 1).toUpperCase())
	...
	public void validate() {
		...
		if (hasEmailId() && hasAlterNateEmailId() && !EmailUtils.validateEmailId(getAlterNateEmailId())) {
